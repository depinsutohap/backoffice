  ...
  $.post('/v1/api/data/stock',{
    ...
    if(e.data.status == '00'){
      for(i=0;i<e.data.list.length;i++){
        $('#data_body').append(
          '<tr class="_click" onclick="_stock_exit_detail(' + e.data.list[i].id + ', ' + e.data.list[i].outlet.id + ')">' +
...
function _stock_exit_detail(id, outlet_id){
  _loading(1)
  $.post('/v1/api/data/stock',{
    data: JSON.stringify({
      id: userData['id'],
    ...
    if(e.data.status == '00'){
      ...
      $('#stock_exit_modal_title').text(e.data.name)
      $('.stock_exit_outlet').text(e.data.outlet.business_id.name + ' - ' + e.data.outlet.name)
      $('.stock_exit_date').text(e.data.date)
      for(i=0;i<e.data.list.length;i++){
        let vid = ''
  ...
  open_sideform('stock_exit_modal')
}

// ========================================================
// FUNCTION FOR ADD STOCK EXIT
// ========================================================


function add_stock_exit(){
...
function _submit_form(){
  $('#form_add_stock_exit').submit(function(e){
    _loading(1);
    e.preventDefault();
    ...
    $.post('/v1/api/data/stock',{
      ...
      if(e.status == '00'){
        sub_href('/page/inventory/stock-exit')
      }
    }).done(function(){
      _loading(0)
    })
