...
// ========================================================
// FUNCTION FOR STOCK TRANSFER LIST
// ========================================================
  ...
  $.post('/v1/api/data/stock',{
    ...
    if(e.data.status == '00'){
      for(i=0;i<e.data.list.length;i++){
        $('#data_body').append(
          '<tr class="_click" onclick="_stock_transfer_detail(' + e.data.list[i].id + ', ' + e.data.list[i].outlet.id + ')">' +
          '<td>' + e.data.list[i].name + '</td>' +
          '<td>' + e.data.list[i].outlet.name + '</td>' +
          '<td>' + e.data.list[i].to_outlet.name + '</td>' +
...
function _stock_transfer_detail(id, outlet_id){
  _loading(1)
  $.post('/v1/api/data/stock',{
    data: JSON.stringify({
      id: userData['id'],
    ...
    if(e.data.status == '00'){
      ...
      $('#stock_transfer_modal_title').text(e.data.name)
      $('.stock_transfer_outlet').text(e.data.outlet.business_id.name + ' - ' + e.data.outlet.name)
      $('.stock_transfer_outlet_to').text(e.data.to_outlet.business_id.name + ' - ' + e.data.to_outlet.name)
      $('.stock_transfer_date').text(e.data.date)
      for(i=0;i<e.data.list.length;i++){
        let vid = ''
  ...
  open_sideform('stock_transfer_modal')
}

// ========================================================
// FUNCTION FOR ADD STOCK TRANSFER
// ========================================================
...
function _submit_form(){
  $('#form_add_stock_transfer').submit(function(e){
    _loading(1);
    e.preventDefault();

    if($('#from_outlet').val() == $('#to_outlet').val()){
      notif('info', 'Don\'t transfer to the same outlet ', 'Please, transfer your inventory to the different outlet');
      _loading(0);
      return false;
    }
    ...
    for(i=0;i<_data.length;i++){
      if($(_data[i]).attr('data-id') == '0'){
        notif('info', 'Fill the empty item field', 'Don\'t forget to fill your item item to add the stock transfer');
        _loading(0);
        return false;
      }else{
        let row = $(_data[i]).attr('data-row'),
            _tu = parseInt($('.total_unit_' + row).val()),
            _detail_data = [$(_data[i]).attr('data-id'), $(_data[i]).attr('data-variant'), '0', _tu]
        if($(_data[i]).attr('data-variant') == 'true'){
          if($('.variant_' + row).val() !== '0'){
            _detail_data[2] = $('.variant_' + row).val();
          }else{
            notif('info', 'Choose the empty variant field', 'Don\'t forget to choose your variant to add the stock transfer');
            _loading(0);
            return false;
          }
        }
    ...
    $.post('/v1/api/data/stock',{
      data: JSON.stringify({
        ...
        status: 9,
        date: $('#date').val(),
        from_outlet: $('#from_outlet').val(),
        to_outlet: $('#to_outlet').val(),
        description: $('#description').val(),
        data_list: _data_list,
      })
    }, function (e) {
      if(e.status == '00'){
        sub_href('/page/inventory/transfer-stock')
      }
    }).done(function(){
      _loading(0)
    })
