import time, ast
import json as _json
from . import api_1
from .. import _auth, Session
from decimal import *
from app.models import *
from sqlalchemy import or_
from datetime import datetime
from sanic.response import json, raw, redirect
from ..models import Hop_User, Hop_Business, Hop_Outlet, Hop_Product_Item, \
    Hop_Role, Hop_Product_Outlet, Hop_Provinces, Hop_Cities,  Hop_Business_Category, \
    Hop_Countries, Hop_Product_Category, Hop_Tax, Hop_Tax_Type, Hop_Special_Promo, Hop_Ap_Detail, \
    Hop_Business, Hop_Num_Emp, Hop_Price, Hop_Measurement_List, Hop_User_Outlet, Hop_Variant_List, \
    Hop_Variant_Category, Hop_Composed_Product, Hop_Inventory, Hop_Inventory_List, \
    Hop_Billing_Package_Item, Hop_Billing_Invoice, Hop_Billing_Payment

# PROMO SECTION's API
@api_1.route('/data/stock', methods=['POST', 'GET'])
@_auth.login_required
async def _api_stock(request):
    # ip_list = request.environ['HTTP_X_FORWARDED_FOR']
    # user_log('/', ip_list)
    if request.method == 'POST':
        apidata = _json.loads(request.form['data'][0])
        response = {}
        user = Hop_User().verify_auth(apidata['id'])
        if user is not None and user.verify_token(apidata['token']):
            # PRODUCT CATEGORY
            if int(apidata['status']) == 0:
                response['data'] = Hop_Inventory()._list(
                    from_date=apidata['from_date'], to_date=apidata['to_date'],
                    outlet_id=apidata['outlet'], owner_id=user.owner_id
                )
                response['status'] = '00'
            elif int(apidata['status']) == 1:
                response['data'] = Hop_Inventory_List()._list(
                    type_id=2, from_date=apidata['from_date'],
                    to_date=apidata['to_date'], outlet_id=apidata['outlet'],
                    owner_id=user.owner_id
                )
                response['status'] = '00'
            elif int(apidata['status']) == 2:
                response['data'] = Hop_Inventory_List()._data(type_id=2, stock_id=apidata['stock_id'], outlet_id=apidata['outlet'], owner_id=user.owner_id)
                response['status'] = '00'
            elif int(apidata['status']) == 3:
                response['data'] = Hop_Inventory_List()._insert_sm(
                    date=apidata['date'], outlet_id=apidata['outlet'],
                    description=apidata['description'],
                    data_list=apidata['data_list'],
                    owner_id=user.owner_id
                )
                response['status'] = '00'
            elif int(apidata['status']) == 4:
                response['data'] = Hop_Inventory_List()._list(
                    type_id=3, from_date=apidata['from_date'],
                    to_date=apidata['to_date'], outlet_id=apidata['outlet'],
                    owner_id=user.owner_id
                )
                response['status'] = '00'
            elif int(apidata['status']) == 5:
                response['data'] = Hop_Inventory_List()._data(type_id=3, stock_id=apidata['stock_id'], outlet_id=apidata['outlet'], owner_id=user.owner_id)
                response['status'] = '00'
            elif int(apidata['status']) == 6:
                response['data'] = Hop_Inventory_List()._insert_se(
                    date=apidata['date'], outlet_id=apidata['outlet'],
                    description=apidata['description'],
                    data_list=apidata['data_list'],
                    owner_id=user.owner_id
                )
                response['status'] = '00'
            elif int(apidata['status']) == 7:
                response['data'] = Hop_Inventory_List()._list(
                    type_id=6, from_date=apidata['from_date'],
                    to_date=apidata['to_date'], outlet_id=apidata['outlet'],
                    owner_id=user.owner_id
                )
                response['status'] = '00'
            elif int(apidata['status']) == 8:
                response['data'] = Hop_Inventory_List()._data(type_id=6, stock_id=apidata['stock_id'], outlet_id=apidata['outlet'], owner_id=user.owner_id)
                response['status'] = '00'
            elif int(apidata['status']) == 9:
                response['data'] = Hop_Inventory_List()._insert_ts(
                    date=apidata['date'], from_outlet_id=apidata['from_outlet'],
                    to_outlet_id=apidata['to_outlet'], description=apidata['description'],
                    data_list=apidata['data_list'],
                    owner_id=user.owner_id
                )
                response['status'] = '00'
            elif int(apidata['status']) == 10:
                response['data'] = Hop_Inventory_List()._list(
                    type_id=7, from_date=apidata['from_date'],
                    to_date=apidata['to_date'], outlet_id=apidata['outlet'],
                    owner_id=user.owner_id
                )
                response['status'] = '00'
            elif int(apidata['status']) == 11:
                response['data'] = Hop_Inventory_List()._data(
                    type_id=7,
                    stock_id=apidata['stock_id'],
                    outlet_id=apidata['outlet'],
                    owner_id=user.owner_id
                )
                response['status'] = '00'

            elif int(apidata['status']) == 12:
                response['data'] = Hop_Inventory()._list(
                    from_date=str(datetime.now().date()),
                    to_date=str(datetime.now().date()),
                    outlet_id=apidata['outlet'],
                    owner_id=user.owner_id
                )
                response['status'] = '00'
            elif int(apidata['status']) == 13:
                response['data'] = Hop_Inventory_List()._insert_so(
                    date=str(datetime.now().date()),
                    outlet_id=apidata['outlet'],
                    description=apidata['description'],
                    data_list=apidata['data_list'],
                    owner_id=user.owner_id
                )
                response['status'] = '00'

            else:
                response['status'] = '50'
                response['message'] = 'Your credential are invalid.'
        return json(response)
    else:
        return redirect(url_for('main.index'))
